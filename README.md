...
```
- there is currently an error running the `npm run dev:seed`, so when using docker, you will need to `npm run dev:docker` -> `docker exec -it <api container id>` -> `npx prisma db seed`
